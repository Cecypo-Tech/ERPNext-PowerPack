// PowerPack - Copy as Message (Sales Invoice)
// Adds Powerup > Copy as Message: copies a short message with a public link to the
// invoice, ready to paste into WhatsApp, SMS or email.
//
...
(() => {
	// Payment details added to the message while the invoice is unpaid.
	// Key by Company name; '*' is used for any company not listed.
	// Example:
	...
	async function copy_message(frm) {
		...
		let message = [
			...
			__('Invoice {0} dated {1}', [doc.name, frappe.datetime.str_to_user(doc.posting_date)]),
			__('Amount Due: {0}', [format_currency(outstanding, doc.currency)]),
			__('Status: {0}', [__(doc.status)]),
			__('View it here: {0}', [url]),
		].join('\n');

		if (outstanding > 0) message += payment_details(doc.company);
