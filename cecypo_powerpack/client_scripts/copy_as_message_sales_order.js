// PowerPack - Copy as Message (Sales Order)
// Adds Powerup > Copy as Message: copies a short message with a public link to the
// order, ready to paste into WhatsApp, SMS or email.
//
...
(() => {
	// Payment details added to the message while the order is neither paid in
	// advance nor fully billed. Key by Company name; '*' is used for any company
	// not listed.
	// Example:
	...
	async function copy_message(frm) {
		...
		const advance = flt(doc.advance_paid);
		const lines = [
			`${doc.customer_name || doc.customer || __('Customer')},`,
			__('Sales Order {0} dated {1}', [doc.name, frappe.datetime.str_to_user(doc.transaction_date)]),
		];
		if (doc.delivery_date) {
			lines.push(__('Delivery Date: {0}', [frappe.datetime.str_to_user(doc.delivery_date)]));
		}
		lines.push(__('Total: {0}', [format_currency(total, doc.currency)]));
		if (advance > 0) lines.push(__('Paid: {0}', [format_currency(advance, doc.currency)]));
		lines.push(__('Status: {0}', [__(doc.status)]));
		lines.push(__('View it here: {0}', [url]));

		let message = lines.join('\n');
		if (advance < total && flt(doc.per_billed) < 100) message += payment_details(doc.company);
